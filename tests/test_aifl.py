...
class TestAIFL(unittest.TestCase):
    ...
    def test_complex_expression(self):
        ...
        expected = (
            "Executed operation: ⇒ on Executed operation: ∧ on Executed symbol: ΔΕ1 and Executed symbol: ΔΘ5α and Executed operation: ∧ on Executed symbol: ΔΜ1 and Executed symbol: ΔΝ2"
        )
    ...
    def test_conditional(self):
        ...
        result = self.executor.execute(expr)
        expected = (
            "Executed conditional: IF Executed operation: > on Executed symbol: ΔΣ1 and Executed symbol: Threshold THEN Executed function: ΔΕ1(Data: SensitiveInfo) ELSE Executed function: ΔΑ1(Data: PublicInfo)"
        )
        self.assertEqual(result.strip(), expected)
    ...
    def test_conjunction(self):
        expr = "ΔΔ1 ∧ ΔΙ5"
        result = self.executor.execute(expr)
        expected = "Executed operation: ∧ on Executed symbol: ΔΔ1 and Executed symbol: ΔΙ5"
        self.assertEqual(result.strip(), expected)
